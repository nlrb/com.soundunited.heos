	...
	send( command, qs, callback ) {
		...
		if( typeof callback === 'function' ) {
			sendQueueItem.callback = ( err, result ) => {
				this._currentSendQueueItem = undefined;
				process.nextTick(() => {
					if (result && result.message && result.message['command under process'] === '') {
						// No result yet, queue for retry
						this.send( command, qs, callback );
					} else {
						callback( err, result );
					}
				});
	...
	connect( callback ) {
		...
		this._socket
		  .setTimeout(3000)
			...
			.once('timeout', () => {
				this.disconnect((self) => self._socket.destroy())
