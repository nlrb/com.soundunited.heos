			...
			sendQueueItem.callback = ( err, result ) => {
				...
				process.nextTick(() => {
					if( result && result.message && result.message.eid ) {
						// there is an error
						err = new Error( result.message.text );
					}
					callback( err, result );
	...
	_nextSendQueueItem() {
		...
		if( this._currentSendQueueItem ) {
			...
			if( writeResult instanceof Error ) {
				...
				this._waitingFor = this._currentSendQueueItem;
				setTimeout(() => {
					if (this._waitingFor && this._waitingFor === this._currentSendQueueItem) {
						this._waitingFor = undefined;
						let cb = this._currentSendQueueItem.callback;
						cb.call( cb, new Error('No response') );
					}
				}, 1000);
			}
		}
